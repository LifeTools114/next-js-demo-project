...
test('입력 방법은 네 줄, 값마다 [복사]', () => {
  ...
  // 상세주소 예문은 「홍길동」 (운영자 26-09-06) — 예시인 채로 복사되면 소포에 홍길동이 적히므로, 그때는 성함 칸으로 안내합니다.
  assert.ok(cap.includes("nm || '홍길동'"), '예문 이름은 홍길동')
  assert.ok(cap.includes('data-needs-name'), '성함 없이는 상세주소를 복사시키지 않습니다')
  assert.ok(!cap.includes("nm || '성함'"), '옛 예문 «성함» 이 남아 있으면 안 됩니다')
})
