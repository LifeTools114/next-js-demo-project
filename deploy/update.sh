...
# 서버에서 도는 대신 읽기 크롬(deploy/setup-worker.sh)이 있으면 확장 코드가 바뀌었을 수 있으니 같이 재시작
if systemctl list-unit-files kb-worker.service >/dev/null 2>&1 && systemctl is-enabled kb-worker >/dev/null 2>&1; then systemctl restart kb-worker; fi
sleep 3
