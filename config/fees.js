 ...
 * 대행 수수료 정책 (두 트랙)
 *
 * 사업 모델: 고객 주문 → 당사가 쿠팡에서 대신 구매(또는 고객이 직접 구매)
 * → 하노이로 배송. 상품 소유권은 고객에게 있고, 당사는 대행 수수료를 받습니다.
 *
 *   구매하고 배송까지(구매대행) — 기본 5,000원 + 초과분
 *   배송만(배송대행)           — 건당 3,000원 (운영자 확정 26-09-04)
 */

export const FEES = {
  /**
   * 배송만(배송대행) 수수료 — 운영자 확정 (26-09-04): 건당 3,000원.
   *
   * 고객이 쿠팡에서 직접 결제하므로 대리 주문은 없지만, 창고 입고 확인·
   * 수령인 매칭·검수·재포장·통관 서류·현지 배송 접수는 그대로 듭니다.
   * 예전에는 배송비 마진($2/kg)으로만 덮었는데, 배송비를 $8/kg 로 내리며
   * (마진 $1/kg) 그 수고를 별도 항목으로 분리했습니다.
   *
   * 상품가·무게에 걸지 않고 **건당 정액**입니다 — 고객이 미리 계산할 수 있게.
   */
  forwardingFeeKrw: 3000,
