    ...
    <>
      ...
      <div className="sheet" style={sheet}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: 4 }}>{doc.title}</div>
          <div style={{ fontSize: 11, color: '#8b95a1', letterSpacing: 2 }}>{doc.titleEn}</div>
        </div>
        ...
        <table style={table}>
          ...
          <tbody>
            ...
            <tr style={{ background: '#f9fafb', fontWeight: 800 }}>
              ...
              <td style={{ ...td, color: '#f04452' }}>{dong(doc.totalVnd)}</td>
            </tr>
          </tbody>
        </table>
        ...
        {isFinal ? (
          <div style={{
            ...
            color: doc.adjust ? '#a05a12' : '#17916b', fontSize: 12, lineHeight: 1.7,
          }}>
            <b>{doc.adjustLabel}</b>
            <div style={{ marginTop: 4, color: '#4e5968' }}>
            ...
            <div style={{ marginTop: 2, color: '#8b95a1', fontSize: 11 }}>
        ...
        {/* 비고 */}
        <div style={{ marginTop: 12, fontSize: 11, color: '#4e5968', lineHeight: 1.8 }}>
          <div style={{ fontWeight: 800, color: '#191f28', marginBottom: 2 }}>안내</div>
...
function Field({ label, value, bold, wide }) {
  return (
    <div style={{ flex: wide ? '1 1 100%' : '1 1 0', minWidth: 0 }}>
      <div style={{ fontSize: 10, color: '#8b95a1' }}>{label}</div>
      <div style={{ fontSize: 12.5, fontWeight: bold ? 800 : 600, color: '#191f28', wordBreak: 'break-all' }}>{value}</div>
    </div>
  )
}
...
const sheet = {
  ...
  boxShadow: '0 2px 12px rgba(0,0,0,.08)', font: '13px/1.6 sans-serif', color: '#191f28',
}
const row = { display: 'flex', gap: 14, flexWrap: 'wrap' }
const table = { width: '100%', borderCollapse: 'collapse', marginTop: 12, fontSize: 12 }
const th = { padding: '7px 10px', borderTop: '1px solid #d7dbe0', borderBottom: '1px solid #d7dbe0', textAlign: 'right', fontSize: 11, color: '#4e5968' }
const td = { padding: '7px 10px', borderBottom: '1px solid #eef1f4', textAlign: 'right' }
