 ...
 */
export function createOrder({ items, zone, customer, track = 'agent', paymentMethod = DEFAULT_METHOD, coupangOrderNo }) {
  ...
  if (frozenQuote.agentLimit?.exceeded) {
    throw new Error(
      `구매대행은 1회 상품가 합계 ${frozenQuote.agentLimit.maxGoodsKrw.toLocaleString('ko-KR')}원까지 접수합니다. 나눠서 신청해 주세요.`,
    )
  }
...
export function findDuplicateOrder({ track, customer, items, coupangOrderNo } = {}) {
  ...
  return match ? { kind: 'same-items', order: match } : null
}
...
export function customerView(order) {
  ...
  return {
     ...
     * 파트너 규격: 세부주소 "K-ECOM(주문자명)" 이 있어야 창고가 접수합니다.
     */
    forwardingGuide:
