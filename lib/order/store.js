 ...
 */
export function createOrder({ items, zone, customer, track = 'agent', paymentMethod = DEFAULT_METHOD, coupangOrderNo, consents }) {
 ...
 *   배송대행  동결 견적의 배송만 수수료 제외 — 26-09-04 부터 배송만도
 *             수수료를 받으므로, 구매대행과 같은 규칙입니다
...
export function cancelOrder(id, { reason, by, customerFault = false } = {}) {
  ...
  if (customerFault && s.netReceivedKrw > 0) {
    ...
    } else if (order.track === 'forwarding' && RETURN_POLICY.forwardingRetainsFee) {
      // 주문에 동결된 값을 씁니다 — 나중에 수수료를 올려도 옛 주문은 옛 금액.
      retainKrw = Math.min(order.quote?.forwardingFeeKrw ?? 0, s.netReceivedKrw)
      retainMemo = '배송만 수수료'
    }
  }
