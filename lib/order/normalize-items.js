 ...
 *   domesticShipKrw · freeShipOverKrw · seller
 *                 국내 배송비와 무료 조건 — 구매대행 청구액에 들어갑니다
 *
...
import { sanitizeImageUrl } from '../peek-result.js'
...
export function normalizeOrderItem(input) {
  return {
    ...
    /**
     * 국내 배송비 (쿠팡 → 한국 창고) — 구매대행 견적에 들어갑니다.
     * 이 값을 버리면 서버 견적이 확장 패널보다 배송비만큼 싸게 나옵니다.
     * 규정은 lib/pricing/domestic.js — 판매자마다 한 번, 무료 조건이면 0원.
     */
    domesticShipKrw: Math.max(0, Math.min(Number(input?.domesticShipKrw) || 0, 50_000)),
    freeShipOverKrw: Math.max(0, Math.min(Number(input?.freeShipOverKrw) || 0, 10_000_000)),
    seller: str(input?.seller, 80),

    productUrl: str(input?.productUrl, 500),
    /** 대표 사진 — 쇼핑몰 그림 서버 주소만 (lib/peek-result.js sanitizeImageUrl). 신청서·주문 화면에서 보여줍니다 */
