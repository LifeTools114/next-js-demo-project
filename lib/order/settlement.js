...
function recalcInner(order, q, actualWeightG) {
  ...
  /**
   * 배송만 수수료(건당 3,000원)는 무게와 무관하므로 실측 후에도 그대로입니다.
   * 여기서 빠뜨리면 최종 청구서가 임시 견적보다 3,000원 적게 나오고,
   * 그 차액이 매번 "환불 대상" 으로 잡힙니다. (26-09-04 신설)
   */
  const forwardingFee = order.track === 'agent' ? 0 : (q.forwardingFeeKrw ?? 0)
  ...
  const subtotal =
    (chargesGoods ? goods + agencyFee : forwardingFee) +
    shipping.totalKrw + surchargeKrw + taxes.total
