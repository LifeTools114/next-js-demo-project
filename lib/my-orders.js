...
/**
 * 주문 API 에 붙일 헤더 — 이 브라우저의 개인 링크 열쇠(+ 운영자 브라우저면 운영자 토큰).
 * 주문번호만으로는 진행 상태만 보이므로, 신청한 브라우저임을 열쇠로 증명합니다.
 */
export function authHeaders() {
  const h = {}
  const key = readMyKey()
  if (key) h['x-my-key'] = key
  try {
    const admin = window.localStorage.getItem('kbeauty-hanoi:admin-token')
