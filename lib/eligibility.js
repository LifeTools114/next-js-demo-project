...
export function checkEligibility(product) {
  ...
  // 전자기기 본체 — 자동 견적하되(기기당 $40 취급비는 할증으로 자동 청구)
  // 한국 기기의 베트남 A/S 불가는 반드시 미리 알립니다.
  const devHay = String(product?.productName || '').toLowerCase().replace(/\s+/g, '')
  const DEVICE_WORDS = ['스마트폰', '휴대폰', '핸드폰', '자급제', '아이폰', '갤럭시탭', '갤럭시북', '아이패드', '태블릿', '노트북', '랩탑', '맥북', '데스크탑', '모니터']
  const DEVICE_EXCLUDE = ['케이스', '파우치', '거치대', '필름', '보호', '충전', '가방', '받침', '스트랩', '링']
  if (DEVICE_WORDS.some((w) => devHay.includes(w)) && !DEVICE_EXCLUDE.some((w) => devHay.includes(w))) {
    warnings.push({
      id: 'device-care',
      message: '전자기기는 기기당 $40 취급비가 추가되며, 한국 기기는 베트남에서 A/S 가 어렵습니다.',
    })
  }
