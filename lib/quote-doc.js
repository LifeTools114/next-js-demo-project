...
import { recalculateWithActualWeight } from './order/settlement.js'
...
function header(order, kind) {
  ...
  return {
    ...
    contact: { label: CONTACT.label, url: CONTACT.kakaoOpenChat, qrPath: CONTACT.qrPath },
    // 주소는 견적서에 싣지 않습니다 (운영자 지시 26-09-01) — 이름·연락처만.
    customer: {
      name: order.customer?.name ?? '',
      phone: order.customer?.phone ?? '',
    },
    trackLabel: order.track === 'agent' ? '구매대행 (상품 대리구매 + 배송)' : '배송대행 (배송만)',
  }
}
...
export function buildProvisionalQuote(order) {
  ...
  return {
    ...
    notes: [
      ...
      `실측 차액이 ${QUOTE.adjustThresholdVnd.toLocaleString('ko-KR')}동 이상일 때만 추가청구 또는 환불하며, 그 미만이면 이 금액 그대로 확정됩니다.`,
...
export function buildFinalQuote(order, debitNote = {}) {
  ...
  /**
   * 조정 여부 — 차액이 임계치 미만이면 임시 견적서 금액을 그대로 청구합니다.
   * 최종 결정(청구·환불 실행)은 운영자가 하며, 이 문서는 판단 근거만 담습니다.
   */
  const adjust = Math.abs(diffVnd) >= QUOTE.adjustThresholdVnd
  const chargeKrw = adjust ? final.total : provisionalKrw

  const rows = [
    ...(order.track === 'agent'
      ? [{ key: 'goods', label: '상품 금액', krw: final.goods },
         { key: 'agency', label: '구매대행 수수료', krw: final.agencyFee }]
      : []),
  ...
  return {
    ...
    thresholdVnd: QUOTE.adjustThresholdVnd,
    totalKrw: chargeKrw,
    totalVnd: round(chargeKrw * rate),
    notes: [
      `임시 견적 ${provisionalKrw.toLocaleString('ko-KR')}원 → 실측 재계산 ${final.total.toLocaleString('ko-KR')}원`,
      adjust
        ? `차액 ${Math.abs(diffVnd).toLocaleString('ko-KR')}동으로 기준(${QUOTE.adjustThresholdVnd.toLocaleString('ko-KR')}동) 이상이라 ${diffKrw > 0 ? '추가 청구' : '환불'} 대상입니다.`
        : `차액 ${Math.abs(diffVnd).toLocaleString('ko-KR')}동으로 기준(${QUOTE.adjustThresholdVnd.toLocaleString('ko-KR')}동) 미만이라 임시 견적서 금액 그대로 청구합니다.`,
    ],
  }
}
