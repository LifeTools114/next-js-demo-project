...
import { enqueue, workerOnline } from './peek-jobs.js'
...
const unescapeHtml = (h) => {
  let out = String(h ?? '').replace(/\\\//g, '/').replace(/\\u002F/gi, '/').replace(/\\x2F/gi, '/').replace(/&amp;/g, '&')
  for (let i = 0; i < 3 && /%[0-9A-F]{2}/i.test(out); i += 1) {
    out = out.replace(/%2F/gi, '/').replace(/%3A/gi, ':').replace(/%3F/gi, '?').replace(/%3D/gi, '=').replace(/%26/gi, '&').replace(/%25/gi, '%')
  }
  return out
}
...
async function findProductLink(url, fetchImpl, signal) {
  ...
  for (let i = 0; i < 6; i += 1) {
    ...
    // 짧은 링크는 PC 브라우저에게는 바로 이동(3xx)을, 폰 브라우저에게는 앱을 열어 보는
    // 「Deeplink Redirect」 중간 페이지(200)를 줍니다 (서버 점검 26-09-07). PC 로 먼저, 안 되면 폰으로.
    let next = null
    for (const ua of [UA_PC, UA]) {
      const res = await fetchImpl(cur, { headers: { ...HEADERS, 'User-Agent': ua }, redirect: 'manual', signal })
      hops.push(`${res.status} ${hostOf(cur)}${ua === UA ? '(폰)' : ''}`)
      const loc = res.headers?.get?.('location')
      if (res.status >= 300 && res.status < 400 && loc) { next = new URL(loc, cur).href; break }
      ...
      if (m) {
        ...
        next = /^\d{5,20}$/.test(hitUrl) ? `https://www.coupang.com/vp/products/${hitUrl}` : new URL(hitUrl, 'https://www.coupang.com').href
        break
      }
    }
    if (!next) return { link: null, hops, fingerprint: fingerprintOf(lastHtml) }
    cur = next
  }
 ...
 */
export async function peekProduct(input, { fetchImpl = globalThis.fetch, timeoutMs = 9000, log = console, readPage = process.env.PRODUCT_PEEK_PAGE === '1' } = {}) {
  ...
  // 「읽기 기기 없음」(page-off) 결과는 기기가 살아난 순간부터 무시합니다 — 크롬을 켠 뒤 같은 상품이 「가격만」으로 남지 않게
  if (hit && Date.now() - hit.at < TTL_MS && !(hit.value?.reason === 'page-off' && workerOnline())) return hit.value
  ...
  const job = peekOnce(first, cacheKey, { fetchImpl, timeoutMs, log, readPage })
  inflight.set(cacheKey, job)
  try { return await job } finally { inflight.delete(cacheKey) }
}

async function peekOnce(first, cacheKey, { fetchImpl, timeoutMs, log, readPage }) {
  ...
  try {
    ...
    if (!link?.productId) {
      // 쇼핑몰 주소인데 상품 번호가 없는 꼴(브랜드관 shop.coupang.com/A00…/…, 기획전) — 읽기 기기가 있으면 그 화면을 열어
      // 진짜 상품 주소를 찾아오게 맡깁니다 (결과 reason:'redirect'). 없으면 5분만 「못 풂」으로 기억합니다.
      if (workerOnline() && /(^|\.)coupang\.com$/.test(hostOf(first.url))) {
        const job = enqueue({ key: cacheKey, url: first.url, productId: null })
        log?.info?.(`[peek] unresolved → worker ${job.id}${trail}`)
        return { ok: false, reason: 'pending', jobId: job.id, productId: null, url: first.url }
      }
      ...
      cache.set(cacheKey, { at: Date.now() - (TTL_MS - 5 * 60_000), value })
      return value
    }
    /**
     * 상품 화면 읽기는 기본으로 끕니다 — 쿠팡이 서버 접속을 봇 차단(Akamai)으로 막아
     * PC 는 403, 폰은 검사 페이지만 옵니다 (서버 점검 26-09-07, 링크 3개 × 변형 4가지 전부).
     * 다시 켜려면 서버 .env.local 에 PRODUCT_PEEK_PAGE=1. 링크 → 상품 번호·정식 주소 확인은 그대로 합니다.
     */
    if (!readPage) {
      // 읽기 기기(사장님 PC 확장·폰)가 살아 있으면 그쪽에 맡기고 「기다리는 중」으로 — 결과는 rememberPeek 로 캐시에 들어옵니다
      if (workerOnline()) {
        const job = enqueue({ key: cacheKey, url: link.url, productId: link.productId, itemId: link.itemId, vendorItemId: link.vendorItemId })
        log?.info?.(`[peek] ${link.productId} → worker ${job.id}${trail}`)
        return { ok: false, reason: 'pending', jobId: job.id, productId: link.productId, url: link.url }
      }
      const value = { ok: false, reason: 'page-off', productId: link.productId, url: link.url }
      log?.info?.(`[peek] ${link.productId} resolved (page-off)${trail}`)
      // 「읽기 기기 없음」은 10초만 기억합니다 — 6시간을 기억하면 크롬을 켠 뒤에도 그 상품은 계속 「가격만」으로 남습니다
      // (운영자 26-09-13 서버 시험에서 실제로 그랬습니다). 같은 순간의 중복 요청만 막으면 됩니다.
      cache.set(cacheKey, { at: Date.now() - (TTL_MS - 10_000), value })
      return value
    }
    ...
    const botPage = /akam|access denied|captcha|Powered and protected/i.test(html.slice(0, 6000)) && !parsed.productPrice
    const blocked = !okStatus || botPage || (/잠시 후 다시|자동입력 방지/i.test(html.slice(0, 4000)) && !parsed.productPrice)
    const value = blocked || (!parsed.productName && !parsed.productPrice)
      ? { ok: false, reason: botPage || status === 403 ? 'bot-protected' : okStatus ? 'unreadable' : `http-${status}`, url: link.url, productId: link.productId }
...
/** 읽기 기기가 가져온 결과를 캐시에 넣습니다 — 같은 상품은 다음부터 바로 나옵니다 */
export function rememberPeek({ productId, itemId = null, vendorItemId = null, url }, value) {
  const key = `${productId}:${itemId ?? ''}:${vendorItemId ?? ''}`
  cache.set(key, { at: Date.now(), value: { ...value, productId, url } })
  return value
}

/** 캐시에 있는 결과 (없으면 null) */
export function peekCached({ productId, itemId = null, vendorItemId = null }) {
  const hit = cache.get(`${productId}:${itemId ?? ''}:${vendorItemId ?? ''}`)
