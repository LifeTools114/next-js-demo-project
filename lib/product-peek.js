...
async function findProductLink(url, fetchImpl, signal) {
  ...
  for (let i = 0; i < 6; i += 1) {
    ...
    if (!m) return { link: null, hops }
    cur = new URL(m[1] ?? m[0], 'https://www.coupang.com').href
  }
...
async function readProduct(link, fetchImpl, signal) {
  ...
  const candidates = [
    `https://www.coupang.com/vp/products/${link.productId}${qs}`,
    `https://m.coupang.com/vm/products/${link.productId}${qs}`,
  ]
  let last = { status: 0, parsed: { productName: '', productPrice: null, spec: null }, html: '', host: '' }
  for (const url of candidates) {
    const res = await fetchImpl(url, { headers: HEADERS, redirect: 'follow', signal })
    const html = res.ok ? await res.text() : ''
    const parsed = res.ok ? parseProductHtml(html) : last.parsed
    last = { status: res.status, parsed, html, host: hostOf(url) }
    if (res.ok && (parsed.productPrice || parsed.productName)) break
  }
  return last
}
...
export async function peekProduct(input, { fetchImpl = globalThis.fetch, timeoutMs = 9000, log = console } = {}) {
  ...
  try {
    const { link, hops } = await findProductLink(first.url, fetchImpl, ctrl.signal)
    const trail = hops.length ? ` hops=[${hops.join(' → ')}]` : ''
    if (!link?.productId) {
      const value = { ok: false, reason: 'unresolved', url: first.url }
      log?.info?.(`[peek] short-link ok=false reason=unresolved${trail}`)
      cache.set(cacheKey, { at: Date.now(), value })
      return value
    }
    const { status, parsed, html, host } = await readProduct(link, fetchImpl, ctrl.signal)
    ...
    log?.info?.(`[peek] ${link.productId} status=${status} via=${host} ok=${value.ok}${value.ok ? '' : ` reason=${value.reason}`}${trail}`)
    cache.set(cacheKey, { at: Date.now(), value })
