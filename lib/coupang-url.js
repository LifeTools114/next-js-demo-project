...
export function parseProductUrl(text) {
  ...
  // /vp/products/번호 · /vm/products/번호 · 제휴 링크의 ?pageKey=번호 (link.coupang.com/re/…)
  const id = url.pathname.match(/\/(?:vp|vm)\/products\/(\d{3,20})/)?.[1]
    ?? (url.searchParams.get('pageKey')?.match(/^\d{3,20}$/)?.[0] ?? null)
