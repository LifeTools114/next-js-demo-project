...
export { calculateShipping, toBillableKg, getRateTable, usdToKrw, roundingRuleText } from './pricing/shipping.js'
...
export function currentPolicy() {
  return {
    ...
    agencyBaseMaxGoodsKrw: FEES.agencyBaseMaxGoodsKrw,
    agencyExcessRate: FEES.agencyExcessRate,
    agencyBaseMaxItems: FEES.agencyBaseMaxItems,
    agencyPerExtraItemKrw: FEES.agencyPerExtraItemKrw,
...
export function applyConfig(config = {}) {
  ...
  for (const key of ['agencyBaseKrw', 'agencyBaseMaxGoodsKrw', 'agencyExcessRate', 'agencyBaseMaxItems', 'agencyPerExtraItemKrw']) {
    if (Number.isFinite(config[key]) && config[key] >= 0) FEES[key] = config[key]
  }
  if (Number.isFinite(config.minOrderGoodsKrw) && config.minOrderGoodsKrw >= 0) {
    ORDER_MIN.goodsKrw = config.minOrderGoodsKrw
  }
...
/**
 * 배송 가능 지역 문구 — 확장 패널이 금액 아래에 굵게 보여줍니다.
 * 운영자 확정 26-09-06: "목록에 있는 도시 외에는 배송이 안 된다. 중부·남부는 현재 안 된다."
 * 도시 목록은 SHIPPING.zones 에서 읽습니다 — 지역을 추가하면 여기 글도 바뀝니다.
 */
...
export { flagSvg } from './ui/flags.js'
export { bannerHtml, BANNER_COPY } from './ui/banner.js'
