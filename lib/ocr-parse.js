...
export function parseShotText(text) {
  ...
  const isPriceLine = (l) => pricesIn(l).some((p) => p.hasComma && /원|\]/.test(l)) || /\d\s?%/.test(l)
  const looksLikeText = (l) => (HANGUL.test(l) || /[A-Za-z]{2,}|\d/.test(l)) && !UI_NOISE.test(l) && !isPriceLine(l)
  const TAIL = /^[\d\s]*(개|매|세트|팩|입|병|캔|봉|묶음)$/
  // 앱은 긴 이름을 두세 줄로 나눕니다 — 첫 한글 줄부터 상품평 줄 전까지 이어 붙입니다
  // (「…약산성 410」에서 잘려 용량·개수를 놓치면 무게가 최소값이 됩니다 — 운영자 캡처 26-09-07)
  const parts = []
  for (let i = from; i < to; i += 1) {
    const l = lines[i]
    if (parts.length === 0) {
      if (HANGUL.test(l) && l.length >= 6 && looksLikeText(l)) parts.push(l)
      continue
    }
    if (!looksLikeText(l) && !TAIL.test(l)) break
    const prev = parts[parts.length - 1]
    // 글자 단위로 잘린 토큰(「410」/「ml」, 「1」/「개」)은 붙여 쓰고, 그 밖에는 띄어 씁니다
    const glue = (/\d$/.test(prev) && /^(ml|mL|g|kg|l|L|리터|개|매|입|병|캔|봉|팩|세트|p|P|ea|EA)/.test(l)) || (TAIL.test(l) && !/^\d/.test(l)) ? '' : ' '
    parts[parts.length - 1] = `${prev}${glue}${l}`
  }
  let productName = parts.join(' ').replace(/^[^가-힣A-Za-z0-9\[(]+/, '').slice(0, 300)
