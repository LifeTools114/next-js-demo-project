...
export function parseProductSpec(productName) {
  ...
  /**
   * 옷 치수를 용량으로 읽지 않기 — "S-5L", "M~4L", "XS/5XL"
   *
   * 26-09-06 사장님 화면: "카라티 (남녀공용 18컬러 S-5L 빅사이즈)" 의 5L 을
   * 5리터로 읽어 티셔츠 한 장이 **5.8kg** 이 되었습니다. 국제배송비만
   * 6만원이 넘게 나와, 아무도 주문할 수 없는 견적이었습니다.
   * 치수 표기는 앞에 다른 치수 글자와 이음표가 옵니다 — 그것으로 가릅니다.
   */
  const isSizeToken = (idx) => /[A-Za-z]{1,3}\s*[-~/]\s*$/.test(text.slice(0, idx))

  // 1) 리터 (2L, 1.5리터)
  const litre = [...text.matchAll(/(\d+(?:[.,]\d+)?)\s*(?:L|리터)(?![a-zA-Z가-힣])/g)]
    .find((m) => !isSizeToken(m.index))
  if (litre) {
