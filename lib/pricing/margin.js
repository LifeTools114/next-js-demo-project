...
export function shippingMargin(shipping, extra = {}) {
  ...
  // 지역 할증 — 고객가는 shipping.zoneSurchargeUsd, 원가는 서버 전용 원가표(지역 7곳, 26-09-06).
  // 모르는 지역은 calculateShipping 이 기본 지역 요금으로 계산하므로 원가도 기본 지역을 씁니다.
  const zoneRevenueUsd = shipping.zoneSurchargeUsd
  const zoneKey = shipping.zone in (SHIPPING.zones ?? {}) ? shipping.zone : SHIPPING.defaultZone
  const zoneCostUsd = round2(COSTS.zoneUsd?.[zoneKey] ?? 0)
