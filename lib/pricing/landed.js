...
export function calculateAgencyFee(goodsKrw, track) {
  if (track !== TRACK.AGENT) return { fee: 0, waived: false, rate: 0, applicable: false }
  if (FEES.agencyWaiverThresholdKrw > 0 && goodsKrw >= FEES.agencyWaiverThresholdKrw) {
    return { fee: 0, waived: true, rate: FEES.agencyRate, applicable: true }
  }
  return {
...
export function quote(items = [], options = {}) {
  ...
  const eligibility = checkCartEligibility(normalized)
  ...
  const taxes = calculateTaxes(normalized, shipping.totalKrw)
  ...
  if (track === TRACK.AGENT) {
    ...
    rows.push({
      key: 'agency',
      label: agency.waived ? '구매대행 수수료 (면제)' : `구매대행 수수료 (${Math.round(agency.rate * 100)}%)`,
      krw: agency.fee,
    })
  }
  rows.push({
  ...
  const rangeAt = (g) => {
    ...
    const tx = calculateTaxes(normalized, sh.totalKrw)
    const base = (track === TRACK.AGENT ? goods + agency.fee : 0) + sh.totalKrw + tx.total
