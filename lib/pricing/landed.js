 ...
 *     청구액 = 상품가 + 대행수수료(기본 5,000원 + 초과분) + 국제배송비
 *     수익: 대행수수료 + 배송 마진
 *
 ...
 * 구매대행 수수료 (배송대행에는 없음)
 *
 * 기본 5,000원(상품가 10만원·5종류까지) + 10만원 초과분 5% + 5종류 초과
 ...
 */
/**
 * 이 주문의 **대행 수수료**(원) — 트랙 상관없이 이것 하나만 부르면 됩니다.
 *
 * 배송만도 26-09-04 부터 건당 3,000원을 받습니다. 예전처럼 트랙별로
 * 따로 계산하면 견적서·장부·정산 중 한 곳이 빠져 금액이 어긋납니다.
 * (실제로 정산에서 빠뜨리면 최종 청구서가 3,000원 적게 나옵니다)
 */
export function serviceFeeKrw(goodsKrw, track, lineCount = 1) {
  if (track === TRACK.AGENT) return calculateAgencyFee(goodsKrw, track, lineCount).fee
  return FEES.forwardingFeeKrw
}
...
export function quote(items = [], options = {}) {
  ...
  // 배송만 수수료 — 고객이 직접 결제해도 입고 확인·검수·재포장·통관 서류는 듭니다.
  const forwardingFee = track === TRACK.FORWARDING ? FEES.forwardingFeeKrw : 0
  ...
  if (forwardingFee > 0) {
    rows.push({ key: 'forwarding', label: '배송만 수수료', krw: forwardingFee })
  }
  rows.push({
    key: 'freight',
  ...
  return {
    ...
    itemSurcharges,
    agency,
    /** 배송만 수수료(원) — 정산·최종 견적서가 이 값을 그대로 씁니다 */
    forwardingFeeKrw: forwardingFee,
    taxes,
