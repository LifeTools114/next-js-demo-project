    ...
    createRequest(order) {
      return {
        ...
        instructions: [
          ...
          `은행: ${env('VN_BANK_NAME') || 'Woori Bank Vietnam (우리은행 베트남)'}`,
          `계좌번호: ${env('VN_BANK_ACCOUNT') || '100153017339'}`,
          // 베트남 은행 이체 화면에는 로마자 예금주명이 뜹니다.
          `예금주: ${env('VN_BANK_HOLDER') || 'PARK SEUNG WOO (박승우)'}`,
  ...
  'manual-bank-krw': {
    ...
    createRequest(order) {
      return {
        ...
        instructions: [
          // 수금 계좌는 청구서에 노출되는 고객 안내 정보입니다. env 로 교체 가능.
          `은행: ${env('KR_BANK_NAME') || 'KB국민은행'}`,
          `계좌번호: ${env('KR_BANK_ACCOUNT') || '33710104041511'}`,
          `예금주: ${env('KR_BANK_HOLDER') || '박승우'}`,
