  ...
  'manual-bank': {
    ...
    createRequest(order) {
      return {
        ...
        instructions: [
          // 수금 계좌는 청구서에 노출되는 고객 안내 정보입니다. env 로 교체 가능.
          // (계좌 변경 — 운영자 확정 26-08-31)
          `은행: ${env('VN_BANK_NAME') || 'Shinhan Bank Vietnam (신한은행 베트남)'}`,
          `계좌번호: ${env('VN_BANK_ACCOUNT') || '700-032-202396'}`,
          // 베트남 은행 이체 화면에는 로마자 예금주명이 뜹니다.
          `예금주: ${env('VN_BANK_HOLDER') || '김영서 (KIM YOUNG SEO)'}`,
          `이체 메모(nội dung)에 주문번호 ${order.orderNo} 를 반드시 입력해 주세요.`,
        ],
    ...
    createRequest(order) {
      return {
        ...
        instructions: [
          // 수금 계좌는 청구서에 노출되는 고객 안내 정보입니다. env 로 교체 가능.
          // (계좌 변경 — 운영자 확정 26-08-31)
          `은행: ${env('KR_BANK_NAME') || '카카오뱅크'}`,
          `계좌번호: ${env('KR_BANK_ACCOUNT') || '3333-02-2739576'}`,
          `예금주: ${env('KR_BANK_HOLDER') || '김영서'}`,
